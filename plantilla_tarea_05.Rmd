...
Esta tarea tiene ejercicios para crear datos de un ensayo clínico (un tipo de experimento) conocido como "Randomized Control Trial". Aquí, pacientes control y en tratamiento se asignan al azar.

Este estudio es sobre pacientes con anorexia nerviosa que presentan pesos por debajo del límite saludable. El éxito del tratamiento en este caso se midio por el aumento de peso. Tenemos dos variables: `pre_kg`, el peso antes del tratamiento, y `kg`, el peso al final del tratamiento. Los pacientes en tratamiento reciben terapia familiar en 20 sesiones.
...
dplyr::glimpse(anorexia)

```

- Respuesta 
  + Casos: [escribir aqui la cantidad de casos, y borrar los corchetes] [4000]
  + Variables: [escribir aqui la cantidad de variables, y borrar los corchetes] [5]
...
- Resultado
  + [Indique con sus palabras el resultado encontrado, en esta sección, y en el siguiente parentesis remplace los XXXX por los estadísticos pertinentes] `(b = XXXX, SE = XXXX, t = XXXX, p < XXXX)`.
...
Vik (2014) llama modelo aumentado al que incluye covariables de interés. Ajusta el modelo aumentado comparando el peso final del grupo control con el de los pacientes tratados. Elige la variable de respuesta y la covariable adecuadas. Luego, muestra el output de la regresión usando `summary()`.
